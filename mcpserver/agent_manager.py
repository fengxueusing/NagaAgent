...
class AgentManager:
    ...
    def _load_global_config(self):
        ...
        try:
            ...
            if config_file.exists():
                ...
                return global_config.get('api', {})
        except Exception as e:
            logger.warning(f"加载全局配置失败: {e}")
        return {}

    def _replace_environment_placeholders(self, text: str, global_api_config: dict) -> str:
        """替换配置中的环境变量占位符，如果环境变量为空则回退到全局配置"""
        if not text:
            return ""
        
        processed_text = str(text)
        
        # 处理常见的占位符，优先使用环境变量，如果为空则使用全局配置
        placeholder_mapping = {
            "{{API_KEY}}": os.getenv('API_KEY') or global_api_config.get('api_key', ''),
            "{{API_BASE_URL}}": os.getenv('API_BASE_URL') or global_api_config.get('base_url', ''),
            "{{DEEPSEEK_API_KEY}}": os.getenv('DEEPSEEK_API_KEY') or global_api_config.get('api_key', ''),
    ...
    def _load_agent_configs(self):
        """从配置文件加载Agent定义"""
        # 加载全局配置
        global_api_config = self._load_global_config()
            ...
            try:
                ...
                for agent_key, agent_data in config_data.items():
                    if self._validate_agent_config(agent_data):
                        # 处理占位符，如果替换后为空则使用全局配置
                        api_base_url = self._replace_environment_placeholders(
                            agent_data.get('api_base_url', ''), global_api_config
                        )
                        api_key = self._replace_environment_placeholders(
                            agent_data.get('api_key', ''), global_api_config
                        )
                        model_provider = self._replace_environment_placeholders(
                            agent_data.get('model_provider', 'openai'), global_api_config
                        )
                        
                        # 如果处理后仍为空，使用全局配置的默认值
                        if not api_base_url:
                            api_base_url = global_api_config.get('base_url', 'https://api.deepseek.com/v1')
                        if not api_key:
                            api_key = global_api_config.get('api_key', '')
                        ...
                        logger.info(f"已加载Agent: {agent_key} ({agent_config.name}) - API: {api_base_url}")
